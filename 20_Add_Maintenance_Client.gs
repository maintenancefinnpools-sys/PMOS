...
function renderAddresses(items){var list=byId('addressList');list.innerHTML='';if(!items.length){hideAddresses();return}items.forEach(function(item){var b=document.createElement('button');b.type='button';b.className='address-option';b.textContent=item.address;b.onclick=function(){byId('address').value=item.address;hideAddresses();queueRecommendations(true)};list.appendChild(b)});list.style.display='block'}
...
byId('frequency').addEventListener('change',scheduleChanged);byId('address').addEventListener('input',queueAddressSuggestions);byId('address').addEventListener('blur',function(){setTimeout(hideAddresses,180)});scheduleChanged();
...
function createMaintenanceClient(input) {
  ...
  const lock=LockService.getDocumentLock();if(!lock.tryLock(10000))throw new Error('Another PMOS operation is running.');let result;
  try{
    const ss=SpreadsheetApp.getActive(),customersSheet=findFirstSheetByName_(ss,['Customers','Customer Database','Customer List']),routeSheet=findFirstSheetByName_(ss,['4-Week Route Template','PMOS 4-Week Route Template','Route Template']);if(!customersSheet||!routeSheet)throw new Error('Customers or 4-Week Route Template sheet was not found.');
    let ct=readHeaderTable_(customersSheet),rt=readHeaderTable_(routeSheet);ensureMaintenanceClientHeaders_(customersSheet,ct,['Customer ID','Calendar Title','Full Address','Primary Phone','Email','Frequency','Service Start Date','Customer Notes']);ensureMaintenanceClientHeaders_(routeSheet,rt,['Customer ID','Calendar Title','Layer','Stop Order']);ct=readHeaderTable_(customersSheet);rt=readHeaderTable_(routeSheet);assertMaintenanceClientNotDuplicate_(ct,name,address,email);
    ...
    appendMappedMaintenanceRow_(customersSheet,ct,values);const weeks=maintenanceWeeksForFrequency_(frequency,firstWeek),days=frequency==='Twice Weekly'?[day,secondDay]:[day],routeRows=[];
    weeks.forEach(week=>days.forEach(serviceDay=>{const layer=`Week ${week} - ${serviceDay}`,stop=requestedStop||nextMaintenanceStopForLayer_(rt,layer),rowValues={'Customer ID':customerId,'Calendar Title':calendarTitle,'Layer':layer,'Route Layer':layer,'Week':week,'Rotation Week':week,'Day':serviceDay,'Weekday':serviceDay,'Stop':stop,'Stop Order':stop,'Order':stop};appendMappedMaintenanceRow_(routeSheet,rt,rowValues);routeRows.push({layer,stop});rt.rows.push(mappedMaintenanceRow_(rt.headers,rowValues));}));SpreadsheetApp.flush();
  ...
  let calendarSummary='Calendar Sync was not available.';try{if(typeof startCalendarSyncFromDate==='function'){const job=startCalendarSyncFromDate(Utilities.formatDate(effectiveDate,PMOS.TIMEZONE,'yyyy-MM-dd'),true);calendarSummary=job&&job.summary?job.summary:'Calendar Sync started automatically.';}}catch(error){calendarSummary=`Client was created, but automatic Calendar Sync reported: ${error}`;}result.summary+='\n\n'+calendarSummary;return result;
}
