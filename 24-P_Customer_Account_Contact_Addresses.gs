...
function pmosWireBillingPanel(prefix){var toggle=document.getElementById(prefix+'Toggle'),panel=document.getElementById(prefix+'Panel'),clear=document.getElementById(prefix+'Clear');if(!toggle||!panel)return;toggle.onclick=function(){panel.classList.add('open');toggle.textContent='Account holder billing address'};if(clear)clear.onclick=function(){pmosRenderBillingAddress(prefix,{enabled:false});window.pmosBillingTouched=true}}
...
var pmosBaseBillingShowAdd=showAdd;showAdd=function(){pmosBaseBillingShowAdd();window.pmosBillingLoaded=false;google.script.run.withSuccessHandler(function(billing){window.pmosBillingLoaded=true;pmosRenderBillingAddress('serviceLocationBilling',billing||{enabled:false})}).getPmosAccountBillingAddress(parentCustomerId)};
