...
assert(context.clientSource.includes("'filterModelNumber','automation'"), 'Water-feature legacy model-number cleanup is missing.');
assert(!context.clientSource.includes('ensureWaterFeatureActuatorFields(card);var label'), 'Water features still inject embedded actuator model fields.');
