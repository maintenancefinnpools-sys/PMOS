...
def main() -> None:
    ...
    equipment_profile_cards = read("24-YZ_Customer_Equipment_Profile_Cards.gs")
    ...
    acceptance_bot = read("25_Acceptance_Test_Bot.gs")
    ...
    require(equipment, "pmos-solar-add-field", "Solar Add Equipment field beside Heater Type", failures)
    require(equipment, "heaterLabel.insertAdjacentElement('afterend',addField)", "Solar Add Equipment placement beside Heater Type", failures)
    require(equipment, "Solar Automation", "Solar automation catalog option", failures)
    require(equipment, "PMOS_SOLAR_CONTROLLER_CATALOG", "solar-specific controller catalog", failures)
    require(equipment, "AquaSolar GL-235", "Hayward solar-specific controller", failures)
    require(equipment, "data-solar-connected", "Solar connection to Equipment Automation", failures)
    forbid(equipment, "data-solar-field=\"notes\"", "per-item Solar Equipment Notes", failures)
    ...
    require(shared_equipment, "function pmosCatalogUnitFieldsHtml", "single shared Pump/Heater catalog field builder", failures)
    require(shared_equipment, "function pmosFilterFieldsHtml", "single shared Filter catalog field builder", failures)
    require(shared_equipment, "function pmosValveActuatorFieldsHtml", "single shared Valve Actuator field builder", failures)
    require(shared_equipment, "pmosCatalogUnitFieldsHtml('pump',suffix,'feature')", "Water Feature pump shared catalog controls", failures)
    require(shared_equipment, "prepareWaterFeatureEquipmentCard(card,defaults||{})", "all Water Features prepared through the shared renderer", failures)
    require(shared_equipment, "function pmosWaterFeatureComponents", "stored and legacy Water Feature component hydration", failures)
    require(shared_equipment, "input.closest('[data-water-feature-equipment]')", "Water Feature components saved only through canonical nested payload", failures)
    require(shared_equipment, ".equipment-grid label", "nested Valve Actuator typography inheritance", failures)
    ...
    require(equipment_profile_cards, "solarHeatingCard", "shared Solar Heating profile card", failures)
    require(equipment_profile_cards, "Connected to Equipment Automation", "Solar Heating automation relationship", failures)
    ...
    require(editor, "result.verified!==true", "Web save read-back verification guard", failures)
    require(lifecycle, "request._pmosFastLifecycle = true", "fast lifecycle save path", failures)
    require(acceptance_bot, "Save transaction reports verified persistence", "edit-save-reopen acceptance coverage", failures)
