...
(function(){
  ...
  if(typeof prepareWaterBodyOptions==='function'){var basePrepare=prepareWaterBodyOptions;prepareWaterBodyOptions=function(root){var result=basePrepare.apply(this,arguments);Array.prototype.forEach.call((root||document).querySelectorAll('.water-body'),function(card){pmosInstallBodyOverview(card);pmosInstallSpaOptions(card);pmosConfigureStandaloneBody(card)});pmosPrepareFilterParts(root||document);pmosTitleCaseHeadings(root||document);return result}}
  if(typeof addWaterBody==='function'){var baseAddBody=addWaterBody;addWaterBody=function(){var result=baseAddBody.apply(this,arguments),cards=document.querySelectorAll('.water-body'),card=cards[cards.length-1];pmosInstallBodyOverview(card);pmosInstallSpaOptions(card);pmosTitleCaseHeadings(card);return result}}
  ...
  if(typeof hydrateBodies==='function'){var baseHydrate=hydrateBodies;hydrateBodies=function(bodies){var result=baseHydrate.apply(this,arguments),rows=bodies||[],cards=document.querySelectorAll('.water-body');Array.prototype.forEach.call(cards,function(card,index){pmosInstallBodyOverview(card);pmosInstallSpaOptions(card);var body=rows[index]||{},shape=card.querySelector('[data-body-field="shape"]'),volume=card.querySelector('[data-body-field="volume"]'),filterModel=card.querySelector('[data-body-field="filterModel"]');if(shape)shape.value=body.shape||'';if(volume)volume.value=body.volume||'';if(filterModel)pmosSyncFilterPartNumber(filterModel,body.filter&&body.filter.partNumber);pmosConfigureStandaloneBody(card);pmosTitleCaseHeadings(card)});return result}}
})();
`;
  };
})();
