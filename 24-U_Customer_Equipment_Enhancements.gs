...
(function(){
  ...
  function pmosSharedAutomationText(pool){if(!pool)return'Uses the Pool equipment and automation selected above.';var parts=[],chem=pool.querySelector('[data-body-field="chemistryEnabled"]'),auto=pool.querySelector('[data-body-field="automationEnabled"]');if(chem&&chem.checked)parts.push([fieldValue(pool,'[data-body-field="chemistryMake"]'),fieldValue(pool,'[data-body-field="chemistryModel"]')].filter(Boolean).join(' '));if(auto&&auto.checked)parts.push([fieldValue(pool,'[data-body-field="automationMake"]'),fieldValue(pool,'[data-body-field="automationModel"]')].filter(Boolean).join(' '));return parts.length?'Uses the same Pool equipment and automation: '+parts.join(' · ')+'.':'Uses the same equipment as the Pool. Automation will follow the Pool selection.'}
  function pmosConfigureSharedBody(card){if(!card)return;var shared=fieldValue(card,'[data-body-field="name"]').toLowerCase()==='spa'&&fieldValue(card,'[data-body-field="equipmentSetup"]')==='Shared with Pool',summary=card.querySelector('[data-pmos-shared-equipment]');if(!summary){summary=document.createElement('div');summary.className='pmos-shared-equipment-summary';summary.setAttribute('data-pmos-shared-equipment','true');var details=card.querySelector('.spa-details');if(details)details.insertAdjacentElement('afterend',summary)}var chemistry=card.querySelector('.chemistry-option'),automation=card.querySelector('.automation-option'),host=card.querySelector('[data-chemistry-equipment-host]');[chemistry,automation,host].forEach(function(node){if(node)node.style.display=shared?'none':''});summary.classList.toggle('open',shared);if(!shared)return;var pool=pmosPrimaryPool(card),chem=card.querySelector('[data-body-field="chemistryEnabled"]'),auto=card.querySelector('[data-body-field="automationEnabled"]');if(chem)chem.checked=!!(pool&&pool.querySelector('[data-body-field="chemistryEnabled"]:checked'));if(auto)auto.checked=!!(pool&&pool.querySelector('[data-body-field="automationEnabled"]:checked'));['chemistryMake','chemistryModel','automationMake','automationModel'].forEach(function(key){var source=pool&&pool.querySelector('[data-body-field="'+key+'"]'),target=card.querySelector('[data-body-field="'+key+'"]');if(source&&target)target.value=source.value});summary.innerHTML='<strong>Shared with Pool</strong><br>'+esc(pmosSharedAutomationText(pool))}
