...
var customerId=${idJson},returnContext=${contextJson},editToken='',loaded=null,originalAddress='',selectedAddress=null,selectedPlacements=[],routeChangeRequested=false,addressTimer=null,addressRequest=0,allRouteResult=null,routeSuggestionLimit=3;function el(id){return document.getElementById(id)}function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(c){return{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})}function norm(v){return String(v||'').toLowerCase().replace(/[^a-z0-9]/g,'')}function setStatus(m,e){el('saveStatus').textContent=m;el('saveStatus').className='save-status'+(e?' error':'')}
