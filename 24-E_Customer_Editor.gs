...
function savePmosCustomerEditorData(input) {
  ...
  try {
    ...
    if (addressChanged && !recommendedPlacements.length) {
      throw new Error('Select a new route placement for the changed address before saving.');
    }
    const requestedContacts = Array.isArray(request.additionalContacts) ? request.additionalContacts.map(function (contact) {
      return {name: String(contact && contact.name || '').trim(), phone: String(contact && contact.phone || '').trim(), email: String(contact && contact.email || '').trim()};
    }) : [];
    requestedContacts.forEach(function (contact) {
      if (!contact.name || !contact.phone) throw new Error('Each added household contact needs a name and phone number.');
    ...
    const routePlan = preparePmosCustomerEditorRouteRows_(customerId, fields, addressChanged ? recommendedPlacements : []);
...
function preparePmosCustomerEditorRouteRows_(customerId, fields, recommendedPlacements) {
  ...
  if (recommendedPlacements && recommendedPlacements.length) {
    ...
    const movingRows = updates.map(function (update) { return update.after.slice(); });
...
function createPmosCustomerEditorHouseholdContacts_(customerId, contacts) {
  ...
  const created = contacts.map(function (contact) {
    ...
    const givenName = parts.join(' ') || String(contact.name || '').trim();
    const person = {names: [{givenName: givenName, familyName: familyName}], phoneNumbers: [{value: contact.phone, type: 'mobile'}],
...
function loadRoutes(){el('routeBox').style.display='block';el('routeMessage').textContent='Address confirmed. Calculating the best route placements…';el('recommendations').innerHTML='';google.script.run.withSuccessHandler(renderRoutes).withFailureHandler(function(e){el('routeMessage').textContent=e&&e.message?e.message:String(e)}).recommendMaintenanceClientRotations({address:el('address').value,addressVerified:true,addressDetails:selectedAddress,frequency:el('frequency').value||'Weekly'})}function renderRoutes(result){var rows=result&&result.recommendations||[];el('routeMessage').textContent=result&&result.qualityMessage||'Select a route placement.';rows.forEach(function(r){var b=document.createElement('button');b.type='button';b.className='rec';b.innerHTML='<b>'+esc(r.label+' · '+r.rotationLabel)+'</b><span>'+esc((r.placements||[]).map(function(p){return 'Week '+p.week+' '+p.day+' · stop '+p.position}).join('; '))+'</span>';b.onclick=function(){Array.prototype.forEach.call(document.querySelectorAll('.rec'),function(x){x.classList.remove('selected')});b.classList.add('selected');selectedPlacements=(r.placements||[]).map(function(p){return{week:p.week,day:p.day,layer:p.layer,position:p.position}});el('routeMessage').textContent='New route placement selected.'};el('recommendations').appendChild(b)})}
function payload(){return{customerId:customerId,editToken:editToken,firstName:el('firstName').value,lastName:el('lastName').value,address:el('address').value,addressVerified:!addressChanged()||!!selectedAddress,addressDetails:selectedAddress,recommendedPlacements:selectedPlacements,additionalContacts:contacts(),phone:el('phone').value,email:el('email').value,calendarTitle:el('calendarTitle').value,status:el('customerStatus').value,frequency:el('frequency').value,serviceStartDate:el('serviceStartDate').value,yearRound:el('yearRound').value,entryInformation:el('entryInformation').value,notes:el('notes').value}}function finishReturn(){google.script.run.withFailureHandler(function(e){setStatus(e&&e.message?e.message:String(e),true)}).returnFromPmosCustomerEditor(customerId,returnContext);setTimeout(function(){google.script.host.close()},350)}el('addContact').onclick=addContact;el('address').addEventListener('input',searchAddress);el('address').addEventListener('blur',function(){setTimeout(function(){el('addressList').style.display='none'},180)});el('frequency').addEventListener('change',function(){if(selectedAddress)loadRoutes()});el('save').onclick=function(){this.disabled=true;setStatus('Saving changes…');google.script.run.withSuccessHandler(function(result){var message=result.contactStatus||'';setStatus((result.calendarStatus==='SCHEDULED'?'Saved · Calendar refresh scheduled':'Saved')+(message?' · '+message:''),/could not/i.test(message));setTimeout(finishReturn,700)}).withFailureHandler(function(e){el('save').disabled=false;setStatus(e&&e.message?e.message:String(e),true)}).savePmosCustomerEditorData(payload())};el('cancel').onclick=finishReturn;google.script.run.withSuccessHandler(fill).withFailureHandler(function(e){setStatus(e&&e.message?e.message:String(e),true)}).getPmosCustomerEditorData(customerId);
</script></body></html>`;
}
