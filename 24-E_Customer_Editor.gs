...
function savePmosCustomerEditorData(input) {
  ...
  if (changedCalendarFields && affectedLayers.length && typeof scheduleAddedMaintenanceCustomerCalendarSync_ === 'function') {
    if (normalizePmosServiceLocationStatus_(request.status) !== 'Active') {
      calendarStatus = 'REVIEW_REQUIRED';
    } else {
      try { scheduleAddedMaintenanceCustomerCalendarSync_(customerId, affectedLayers, {serviceLocationId: ''}); calendarStatus = 'SCHEDULED'; }
...
function preparePmosCustomerEditorRouteRows_(customerId, fields, recommendedPlacements) {
  ...
  table.rows.forEach(function (row, index) {
    ...
    if (serviceLocationIndex >= 0 && String(row[serviceLocationIndex] || '').trim()) return;
  ...
  if (recommendedPlacements && recommendedPlacements.length) {
    ...
    const remainingRows = table.rows.filter(function (row) {
      if (String(row[idIndex] || '').trim() !== customerId) return true;
      return serviceLocationIndex >= 0 && Boolean(String(row[serviceLocationIndex] || '').trim());
...
function preparePmosCustomerEditorEquipment_(customerId, calendarTitle, equipmentValues) {
  ...
  for (let index = 0; index < values.length; index++) {
    if (String(values[index][0] || '').trim() === customerId &&
        !String(values[index][5] || '').trim()) {
      return {sheet: sheet, rowNumber: index + 1, before: values[index].slice(0, 7), after: [customerId, calendarTitle, equipmentValues.summary, equipmentValues.detailsJson, new Date(), '', '']};
    }
  }
