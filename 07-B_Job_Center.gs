...
function renderDefinition(){const item=definition();$('jobName').textContent=item.label;$('jobDescription').textContent=item.description;$('calendarOptions').style.display=item.mode==='runtime'?'block':'none';$('start').textContent=item.mode==='runtime'?'Start / Continue':item.mode==='repair'?'Open Repair':'Run';$('pause').style.display=item.mode==='runtime'&&isActive(currentState)?'inline-block':'none';}
function setBusy(value){busy=value;$('start').disabled=value;$('pause').disabled=value;$('refresh').disabled=value;}
...
function isActive(state){return Boolean(state&&state.type&&['Complete','Cancelled','Idle','Paused','Paused on error'].indexOf(state.status)<0);}
...
async function runRuntime(){setBusy(true);lastError='';$('error').textContent='';try{renderRuntime(await server('startVerifiedCalendarSyncJob',$('autoMode').checked,{includeStartedToday:$('includeStartedToday').checked}));}catch(error){lastError=error.message;$('error').textContent=lastError;}finally{setBusy(false);}}
