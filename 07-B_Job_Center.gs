...
function renderDefinition(){const item=currentDefinition();$('selectionNote').textContent='Selected operation: '+item.label;$('jobName').textContent=item.label;$('jobDescription').textContent=item.description;$('autoLabel').style.display=item.resumable?'block':'none';$('runOnce').style.display=item.resumable?'inline-block':'none';$('pause').style.display=item.resumable?'inline-block':'none';$('start').textContent=item.resumable?'Start / Continue':'Run';}
...
function renderCalendarStatus(status){const current=status||{};$('status').textContent=current.status||'Idle';$('remaining').textContent=current.remaining==null?'—':current.remaining;$('processed').textContent=(current.lastCreated||0)+' / '+(current.lastUpdated||0)+' / '+(current.lastDeleted||0);$('lastRun').textContent=current.lastRunAt||'—';if(Number(current.remaining||0)===0&&current.status==='Complete'){$('summary').textContent='Calendar is already synchronized.';}else if(current.status==='Waiting'){$('summary').textContent='Calendar Sync saved its progress and is waiting for the next automatic continuation.';}else if(current.status==='Running'){$('summary').textContent='Calendar Sync is running until the safe execution-time limit.';}else if(current.status==='Paused'){$('summary').textContent='Calendar Sync is paused.';}else{$('summary').textContent='Calendar Sync status: '+String(current.status||'Idle')+'.';}$('error').textContent=lastActionError||current.lastError||'';}
...
async function startCalendar(){setBusy(true);lastActionError='';$('error').textContent='';try{const auto=$('autoMode').checked;const status=auto?await server('startCalendarAutoContinue'):await runCalendarOnce();renderCalendarStatus(status);}catch(error){lastActionError=error.message;$('error').textContent=lastActionError;}finally{setBusy(false);}}
async function runCalendarOnce(){await server('runCalendarSyncBatchNow');return await server('getCalendarAutoSyncStatus');}
...
$('start').onclick=execute;$('runOnce').onclick=async()=>{setBusy(true);lastActionError='';$('error').textContent='';try{renderCalendarStatus(await runCalendarOnce());}catch(error){lastActionError=error.message;$('error').textContent=lastActionError;}finally{setBusy(false);}};$('pause').onclick=pause;$('history').onclick=()=>server('showPmosJobHistory').catch(error=>{lastActionError=error.message;$('error').textContent=lastActionError;});
