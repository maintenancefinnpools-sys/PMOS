...
async function runRuntime(){try{setBusy(true);clearError();const state=await server(isPaused(currentState)?'resumeSync':'startSync');renderRuntime(state);}catch(error){showError(error);}finally{setBusy(false);}}
