/**
 * PMOS Job Center UI.
 * Calendar Sync uses the established Calendar continuation worker.
 * One-off tasks use their task providers.
 */
...
<div class="status-grid"><div class="metric"><span>Status</span><strong id="status">Idle</strong></div><div class="metric"><span>Remaining</span><strong id="remaining">—</strong></div><div class="metric"><span>Created / Updated / Removed</span><strong id="processed">0 / 0 / 0</strong></div><div class="metric"><span>Last run</span><strong id="lastRun">—</strong></div></div>
<div class="summary" id="summary">No job is currently active.</div><div class="error" id="error"></div>
<label id="autoLabel" style="display:block;margin-top:12px"><input type="checkbox" id="autoMode" checked> Continue automatically until complete</label>
<div class="actions"><button class="action primary" id="start">Start / Continue</button><button class="action secondary" id="runBatch">Run one batch</button><button class="action danger" id="pause">Pause</button><button class="action secondary" id="history">History</button></div>
</div></div>
<script>
const definitions=[
...
function renderJobs(){$('jobs').innerHTML=definitions.map(item=>'<button class="job'+(item.type===selectedType?' selected':'')+'" data-type="'+item.type+'" aria-pressed="'+(item.type===selectedType?'true':'false')+'">'+item.label+(item.type===selectedType?' — Selected':'')+'</button>').join('');document.querySelectorAll('.job').forEach(button=>{button.onclick=()=>{selectedType=button.dataset.type;lastActionError='';server('rememberPmosJobType',selectedType).catch(()=>{});renderJobs();renderDefinition();if(currentDefinition().resumable){refresh();}else{showReadyState();}};});}
function renderDefinition(){const item=currentDefinition();$('selectionNote').textContent='Selected operation: '+item.label;$('jobName').textContent=item.label;$('jobDescription').textContent=item.description;$('autoLabel').style.display=item.resumable?'block':'none';$('runBatch').style.display=item.resumable?'inline-block':'none';$('pause').style.display=item.resumable?'inline-block':'none';$('start').textContent=item.resumable?'Start / Continue':'Run';}
function setBusy(value){busy=value;['start','runBatch','pause'].forEach(id=>$(id).disabled=value);}
...
function renderCalendarStatus(status){const current=status||{};$('status').textContent=current.status||'Idle';$('remaining').textContent=current.remaining==null?'—':current.remaining;$('processed').textContent=(current.lastCreated||0)+' / '+(current.lastUpdated||0)+' / '+(current.lastDeleted||0);$('lastRun').textContent=current.lastRunAt||'—';if(Number(current.remaining||0)===0&&current.status==='Complete'){$('summary').textContent='Calendar is already synchronized.';}else if(current.status==='Waiting'){$('summary').textContent='Calendar Sync is waiting for the next automatic batch.';}else if(current.status==='Running'){$('summary').textContent='Calendar Sync is running.';}else if(current.status==='Paused'){$('summary').textContent='Calendar Sync is paused.';}else{$('summary').textContent='Calendar Sync status: '+String(current.status||'Idle')+'.';}$('error').textContent=lastActionError||current.lastError||'';}
async function refresh(){if(!currentDefinition().resumable)return;try{renderCalendarStatus(await server('getCalendarAutoSyncStatus'));}catch(error){lastActionError=error.message;$('error').textContent=lastActionError;}}
async function startCalendar(){setBusy(true);lastActionError='';$('error').textContent='';try{const auto=$('autoMode').checked;const status=auto?await server('startCalendarAutoContinue'):await runOneCalendarBatch();renderCalendarStatus(status);}catch(error){lastActionError=error.message;$('error').textContent=lastActionError;}finally{setBusy(false);}}
async function runOneCalendarBatch(){await server('runCalendarSyncBatchNow');return await server('getCalendarAutoSyncStatus');}
...
async function execute(){if(currentDefinition().resumable){await startCalendar();}else{await executeTask();}}
async function pause(){setBusy(true);lastActionError='';$('error').textContent='';try{renderCalendarStatus(await server('pauseCalendarAutoContinue'));}catch(error){lastActionError=error.message;$('error').textContent=lastActionError;}finally{setBusy(false);}}
$('start').onclick=execute;$('runBatch').onclick=async()=>{setBusy(true);lastActionError='';$('error').textContent='';try{renderCalendarStatus(await runOneCalendarBatch());}catch(error){lastActionError=error.message;$('error').textContent=lastActionError;}finally{setBusy(false);}};$('pause').onclick=pause;$('history').onclick=()=>server('showPmosJobHistory').catch(error=>{lastActionError=error.message;$('error').textContent=lastActionError;});
...
</script></body></html>`).setWidth(820).setHeight(610);
