...
<div class="summary" id="summary">Select an operation.</div><div class="progress"><div class="bar" id="progressBar"></div></div><div id="refreshNote" class="refresh-note"></div><div class="error" id="error"></div>
...
function finishButton(button,text){if(!button)return;button.classList.remove('opening');button.classList.add('success');button.textContent=text||'Done';setTimeout(()=>{button.classList.remove('success');button.disabled=false;button.textContent=button.dataset.label||button.textContent;renderDefinition();},700);}
function restoreButton(button){if(!button)return;button.classList.remove('opening');button.disabled=false;button.textContent=button.dataset.label||button.textContent;}
...
async function refreshRuntime(showFeedback){const button=$('refresh');if(showFeedback)beginButton(button,'Refreshing…');$('refreshNote').textContent='Refreshing status…';try{renderRuntime(await server('getReviewedCalendarSyncJobCenterStatus'));$('refreshNote').textContent='Last refreshed '+new Date().toLocaleTimeString();if(showFeedback)finishButton(button,'Refreshed');}catch(error){lastError=error.message;$('error').textContent=lastError;$('refreshNote').textContent='Refresh failed';if(showFeedback)restoreButton(button);}}
...
$('start').onclick=execute;$('pause').onclick=pause;$('refresh').onclick=()=>definition().mode==='runtime'?refreshRuntime(true):(beginButton($('refresh'),'Refreshing…'),setTimeout(()=>{showReady();finishButton($('refresh'),'Refreshed');$('refreshNote').textContent='Last refreshed '+new Date().toLocaleTimeString();},180));$('history').onclick=openHistory;$('close').onclick=function(){beginButton($('close'),'Closing…');setTimeout(()=>google.script.host.close(),120);};
