...
<body>
  ...
  <div class="layout">
    <div class="jobs">
      ...
      <button type="button" class="job" data-type="RECONCILE_FUTURE" onclick="selectJob(this)">Reconcile Calendar</button>
      ...
      <button type="button" class="job" data-type="MAP_EXPORT" onclick="selectJob(this)">Export Map Layers</button>
    </div>
    <div class="panel">
      ...
      <div class="buttons">
        ...
        <button id="closeButton" class="action secondary" type="button" onclick="google.script.host.close()">Close</button>
      </div>
    </div>
  </div>
<script>
  ...
  var jobs={
    ...
    CALENDAR_SYNC:{label:'Calendar Sync',purpose:'Update Google Calendar events based on the current PMOS 4-Week Route Template.',supportsAuto:true,runLabel:'Run One Batch'},
    RECONCILE_FUTURE:{label:'Reconcile Calendar',purpose:'Safely remove and replace all future PMOS Calendar events from the selected effective date using the current 4-Week Route Template while preserving historical events.',supportsAuto:false,runLabel:'Apply Reconciliation'},
    CALENDAR_REPAIR:{label:'Calendar Repair',purpose:'Add, remove, or rearrange Calendar visits within a selected date range before applying temporary repairs.',supportsAuto:false,runLabel:'Apply Previewed Repair'},
    CUSTOMER_SYNC:{label:'Customer Database Sync',purpose:'Detect changes in the Customers sheet and update matching customer-information fields throughout the PMOS 4-Week Route Template.',supportsAuto:false,runLabel:'Run Customer Sync'},
    MAP_EXPORT:{label:'Export Map Layers',purpose:'Generate current route-layer CSV files from the PMOS 4-Week Route Template for import into Google My Maps.',supportsAuto:false,runLabel:'Export Map Layers'}
  };
