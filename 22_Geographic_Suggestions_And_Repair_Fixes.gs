/**
 * Geography-first maintenance placement and Calendar Repair runtime fixes.
 */
...
function maintenanceGeocodeAddress_(address) {
  ...
  const key = 'PMOS_GEO_' + Utilities.base64EncodeWebSafe(
    Utilities.computeDigest(Utilities.DigestAlgorithm.MD5, text.toLowerCase())
  ).slice(0, 40);
...
function maintenanceRotationGeography_(table, day, candidateWeeks, targetPoint) {
  ...
  table.rows.forEach(row => {
    ...
    if (!match || candidateWeeks.indexOf(Number(match[1])) < 0 || layer.toLowerCase().indexOf(day.toLowerCase()) < 0) return;
    stops++;
    ...
    if (distance != null && Number.isFinite(distance)) distances.push(distance);
  });
  distances.sort((a, b) => a - b);
  const nearest = distances.length ? distances[0] : null;
  const nearby = distances.filter(value => value <= 8).length;
  const median = distances.length ? distances[Math.floor(distances.length / 2)] : null;
  return {weeks:candidateWeeks.slice(), stops, nearest, nearby, median, geocoded:distances.length};
}

function compareMaintenanceGeography_(a, b) {
  const aHas = a.nearest != null, bHas = b.nearest != null;
  if (aHas !== bHas) return aHas ? -1 : 1;
  if (aHas && bHas) {
    if (a.nearby !== b.nearby) return b.nearby - a.nearby;
    if (Math.abs(a.nearest - b.nearest) > 0.75) return a.nearest - b.nearest;
    if (a.median != null && b.median != null && Math.abs(a.median - b.median) > 1.5) return a.median - b.median;
  }
...
function suggestMaintenanceClientPlacement(input) {
  ...
  if (frequency === 'Weekly' || frequency === 'Twice Weekly') {
    return {week:1, summary: frequency === 'Twice Weekly'
  ...
  const geographyText = best.nearest != null
    ? `Nearest existing ${day} stop is approximately ${best.nearest.toFixed(1)} km away; ${best.nearby} existing stop(s) are within about 8 km.`
    : 'PMOS could not geocode enough route addresses, so stop count was used as the fallback.';
  const rotationText = best.weeks.length === 2 ? `Weeks ${best.weeks[0]} and ${best.weeks[1]}` : `Week ${best.weeks[0]}`;
  return {
    week: best.weeks[0],
    summary: [
...
function repairRouteIdentityPool_() {
  ...
  readRoutesInPhysicalOrder_().forEach(row => {
    if (row.customerId) identities.ids[String(row.customerId).trim()] = true;
    if (row.title) identities.titles[normalize_(row.title)] = true;
  });
  return identities;
}
...
function repairEventMatchesRoute_(event, identities) {
  ...
  if (idMatch && identities.ids[String(idMatch[1]).trim()]) return true;
  return Boolean(identities.titles[normalize_(event.getTitle())]);
}
...
function repairCalendarManagedDayEvents_(calendar, dateText) {
  ...
  return calendar.getEvents(bounds.start, bounds.end)
    ...
    .filter(event => isPmosManagedCalendarEvent_(event) || repairEventMatchesRoute_(event, identities))
...
function repairCalendarSixAmTimeForOrder_(visitDate, order, settings) {
  ...
  return new Date(sixAm.getTime() + (normalTarget.getTime() - normalFirst.getTime()));
}
...
function repairCalendarApplyDay_(calendar, dateText, repairItems, settings) {
  ...
  repairItems.forEach(item => {
    if (existingEvents.some(event => repairCalendarEventAlreadyRepresentsItem_(event, item))) skipped++;
  ...
  let created = 0, repositioned = 0;
  const errors = [];
  sequence.forEach((entry, index) => {
    const combinedOrder = index + 1;
    const targetStart = repairCalendarSixAmTimeForOrder_(visitDate, combinedOrder, settings);
    const targetEnd = new Date(targetStart.getTime() + settings.eventDurationMinutes * 60000);
    try {
      if (entry.kind === 'existing') {
        const event = entry.event;
        if (event.getStartTime().getTime() !== targetStart.getTime() || event.getEndTime().getTime() !== targetEnd.getTime()) {
      ...
      const description = [item.description,'','PMOS_HISTORY_REPAIR=true',item.customerId ? `PMOS_CUSTOMER_ID=${item.customerId}` : '',`PMOS_REPAIR_ORIGINAL_LAYER=${item.layer || ''}`,`PMOS_REPAIR_APPLIED_DATE=${item.date}`,`PMOS_REPAIR_STOP_ORDER=${combinedOrder}`].filter(Boolean).join('\n');
      const event = calendar.createEvent(item.title, targetStart, targetEnd, {description, location:item.address || ''});
