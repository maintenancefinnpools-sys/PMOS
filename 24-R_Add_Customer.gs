...
<div class="shell">
  ...
  <section class="section-card">
    <div class="section-head"><div><h3>Customer Contact Info</h3><p>The first Account Contact is primary and defines the customer identity.</p></div></div>
    <div class="grid">
      <div class="field"><label>Last name<input id="lastName" autocomplete="family-name" autocapitalize="words"></label></div>
      <div class="field"><label>First name<input id="firstName" autocomplete="given-name" autocapitalize="words"></label></div>
      <div class="field"><label>Phone <span class="optional">optional</span><input id="phone" autocomplete="tel" inputmode="tel" maxlength="14" placeholder="(###) ###-####"></label></div>
      ...
      <div class="wide" style="padding-top:4px;border-top:1px solid #e1e7ea"><div class="subhead" style="margin-top:9px">Account Contacts</div><div id="accountContacts" class="account-contact-list"></div><button id="addAccountContact" class="contact-action" type="button" style="margin-top:8px">+ Add Account Contact</button></div>
    </div>
  </section>
  ...
  <section class="section-card">
    ...
    <div class="water-body-shell"><div class="water-body-helper helper">Enter the main body first, then add a body of water or specialized equipment only when needed.</div><div id="waterBodies" class="water-bodies"></div><div class="water-body-actions"><button id="addBodyButton" type="button" class="inline-button">+ Add Body of Water</button></div></div>
  </section>
  ...
  <section class="section-card">
    ...
    <div class="grid">
      ...
      <div class="field wide"><label>General Notes <span class="optional">optional</span><textarea id="notes"></textarea></label></div>
      <div class="field"><label>Opening Notes <span class="optional">optional</span><textarea id="openingNotes"></textarea></label></div>
      <div class="field"><label>Closing Notes <span class="optional">optional</span><textarea id="closingNotes"></textarea></label></div>
    </div>
  </section>
...
function addCustomerFormatName(input){if(typeof formatPmosTitleCaseInput==='function'){formatPmosTitleCaseInput(input);return}if(window.pmosTitleCaseInput){window.pmosTitleCaseInput(input);return}var start=input.selectionStart,end=input.selectionEnd,value=String(input.value||''),formatted=value.replace(/(^|[\\s'’-])([a-zà-öø-ÿ])/g,function(match,prefix,letter){return prefix+letter.toUpperCase()});if(formatted!==value){input.value=formatted;if(start!=null&&input.setSelectionRange)input.setSelectionRange(start,end)}}
...
try{initializeAddCustomer()}catch(initializationError){
  addCustomerSetStatus('Some shared controls did not initialize. Core customer fields remain available; refresh PMOS if an Add button reports an error.','error');
  var fallbackFirst=byId('firstName'),fallbackLast=byId('lastName'),fallbackPhone=byId('phone'),fallbackLocation=byId('serviceLocationName');
  fallbackFirst.oninput=function(){addCustomerFormatName(fallbackFirst);updateAccountPreview()};
  fallbackLast.oninput=function(){addCustomerFormatName(fallbackLast);updateAccountPreview();updatePrimaryLocationName()};
  fallbackPhone.oninput=function(){if(typeof formatPmosPhoneInput==='function')formatPmosPhoneInput(fallbackPhone)};
  fallbackLocation.oninput=function(){addCustomerFormatName(fallbackLocation);serviceLocationNameEdited=true};
  byId('addAccountContact').onclick=function(){if(typeof pmosAddAccountContact==='function')pmosAddAccountContact('accountContacts',{});else addCustomerSetStatus('Account Contact controls did not load. Refresh PMOS and try again.','error')};
  byId('addServiceLocationContact').onclick=function(){if(typeof pmosAddLocationContact==='function')pmosAddLocationContact('serviceLocationContacts',{});else addCustomerSetStatus('Service Location Contact controls did not load. Refresh PMOS and try again.','error')};
  byId('addBodyButton').onclick=function(){if(typeof addWaterBody==='function'){addWaterBody();if(typeof prepareWaterBodyOptions==='function')prepareWaterBodyOptions(byId('waterBodies'))}else addCustomerSetStatus('Body of Water controls did not load. Refresh PMOS and try again.','error')};
  byId('saveButton').onclick=createAddCustomer;
  byId('openProfileButton').onclick=openCreatedCustomerProfile;
  byId('cancelButton').onclick=function(){google.script.host.close()};
}
