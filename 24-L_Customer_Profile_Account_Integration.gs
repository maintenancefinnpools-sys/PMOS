 ...
 *
 * This module uses explicit runtime entry points rather than top-level monkey-patching so
 * Apps Script file execution order cannot prevent the integration from loading.
 */

function showPmosCustomerAccountLookup(initialCustomerId) {
  const html = HtmlService.createHtmlOutput(
    buildPmosCustomerAccountLookupHtml_('LOOKUP', initialCustomerId)
  ).setWidth(1180).setHeight(780);
  SpreadsheetApp.getUi().showModelessDialog(html, 'Customer Lookup');
}

function searchPmosCustomerAccountProfiles(query) {
  const baseRows = searchPmosCustomerProfiles('');
  const byCustomerId = {};
  ...
  return listPmosCustomerAccountsForServiceLocations('').map(function(account) {
    ...
    return {
      ...
      accountSearchText: normalizePmosCustomerSearch_([
        ...
        account.address,
        account.phone,
        (accountTerms[account.accountId] || []).join(' ')
      ].join(' '))
    };
  }).filter(function(row) {
...
function selectAccountServiceLocation(customerId){if(!customerId||String(customerId)===String(selectedId))return;selectedId=customerId;el('profile').style.display='none';google.script.run.withSuccessHandler(renderProfile).withFailureHandler(showError).getPmosCustomerAccountProfile(customerId)}
...
function showPmosCustomerAccountEditor(customerId, returnContext) {
...
function buildPmosCustomerAccountEditorHtml_(customerId, returnContext) {
  let html = buildPmosCustomerEditorHtml_(customerId, returnContext);
...
function showPmosAddServiceLocation(customerId) {
  ...
  htmlText = htmlText.split('.createPmosAdditionalServiceLocation(').join('.createPmosAdditionalServiceLocationForAccount(');
  htmlText = htmlText.split('.showPmosCustomerEditor(').join('.showPmosCustomerAccountEditor(');
  htmlText = htmlText.replace(
