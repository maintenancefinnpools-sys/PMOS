...
function pmosAddAccountContact(containerId,contact){var root=document.getElementById(containerId);if(!root)return;var row=pmosAccountContactRow(contact||{});root.appendChild(row);var first=row.querySelector('[data-account-contact="firstName"]');if(first)first.focus()}
