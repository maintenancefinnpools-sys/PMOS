...
function readPmosAdditionalServiceLocations_(customerId) {
  const sheet = SpreadsheetApp.getActive().getSheetByName(PMOS_SERVICE_LOCATIONS_SHEET_);
...
function savePmosServiceLocation(input) {
  ...
  if (affectedLayers.length && typeof scheduleAddedMaintenanceCustomerCalendarSync_ === 'function') {
    if (status !== 'Active') {
      ...
      try {
        scheduleAddedMaintenanceCustomerCalendarSync_(customerId, affectedLayers, {serviceLocationId: locationId});
...
function applyPmosServiceLocationRouteFields_(headers, row, record) {
  ...
  pmosServiceLocationSetRouteValue_(headers, row, ['Sanitization Type(s)'], record.equipmentValues && record.equipmentValues.sanitization || '');
  pmosServiceLocationSetRouteValue_(headers, row, ['Automation'], record.equipmentValues && record.equipmentValues.automation || '');
  return row;
}
