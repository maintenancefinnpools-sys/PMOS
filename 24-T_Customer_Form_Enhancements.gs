...
(function(){
  ...
  function makeNoteField(id,label,prototype){var node;if(prototype&&prototype.matches&&prototype.matches('label')){node=document.createElement('label');node.className=(prototype.className||'full')+' pmos-note-field';node.appendChild(document.createTextNode(label));var area=document.createElement('textarea');area.id=id;node.appendChild(area);return node}node=document.createElement('div');node.className='field wide pmos-note-field';node.innerHTML='<label>'+label+' <span style="font-weight:400;text-transform:none;letter-spacing:0">optional</span><textarea id="'+id+'"></textarea></label>';return node}
