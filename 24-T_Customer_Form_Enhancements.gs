...
function readPmosCustomerCategorizedNotes_(customerId) {
  ...
  if (!id) return {generalNotes: '', equipmentNotes: '', openingNotes: '', closingNotes: '', maintenanceNotes: ''};
...
function normalizePmosSolarEquipment_(input) {
  return (Array.isArray(input) ? input : []).slice(0, 20).map(function(item) {
    ...
    return item.make || item.model || item.modelNumber || item.quantity || item.notes;
  });
}

function pmosCustomerFormEnhancementStyles_() {
  return '.pmos-note-field textarea,.pmos-equipment-notes textarea{min-height:70px;resize:vertical}.pmos-equipment-notes{grid-column:1/-1;margin-top:10px;padding-top:10px;border-top:1px solid #dce5e8}.pmos-equipment-notes label{display:flex;flex-direction:column;gap:5px;color:#6f7d84;font-size:10px;font-weight:900;letter-spacing:.07em;text-transform:uppercase}.pmos-equipment-notes textarea{width:100%;padding:9px 10px;border:1px solid #bfcbd1;border-radius:7px;background:#fff;color:#293944;font:inherit;outline:none}.pmos-solar-panel{display:none;grid-column:1/-1;margin-top:-2px;padding:0 0 8px}.pmos-solar-panel.open{display:block}.pmos-solar-head{display:flex;align-items:center;justify-content:space-between;gap:10px}.pmos-solar-title{font-size:11px;font-weight:900}.pmos-solar-connected{display:flex;align-items:center;gap:7px;color:#40535d;font-size:10px;font-weight:800;letter-spacing:0;text-transform:none}.pmos-solar-connected input{width:auto}.pmos-solar-add-field{display:none}.pmos-solar-add-field.open{display:flex}.pmos-solar-add{width:100%;padding:8px 9px;border:1px solid #9db6c1;border-radius:7px;background:#fff;color:#0f5470;font:inherit;font-size:11px;font-weight:700}.pmos-solar-list{display:grid;gap:6px;margin-top:7px}.pmos-solar-equipment-card{position:relative;display:grid;grid-template-columns:120px minmax(0,1fr);gap:8px;align-items:end;padding:7px;border:1px solid #d5e0e5;border-radius:7px;background:#fff}.pmos-solar-equipment-head{display:block;min-height:24px;padding-right:58px;margin:0;font-size:10px;font-weight:900}.pmos-solar-equipment-head .remove-button{position:absolute;right:7px;top:7px;padding:4px 6px;font-size:9px}.pmos-solar-equipment-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px}.pmos-solar-equipment-grid label{display:flex;flex-direction:column;gap:4px;color:#6f7d84;font-size:8px;font-weight:900;letter-spacing:.05em;text-transform:uppercase}.pmos-solar-equipment-grid input{width:100%;padding:7px 8px;border:1px solid #bfcbd1;border-radius:6px;background:#fff;color:#293944;font:inherit;font-size:11px}.pmos-solar-section-notes{display:flex;flex-direction:column;gap:4px;margin-top:8px;color:#6f7d84;font-size:9px;font-weight:900;letter-spacing:.05em;text-transform:uppercase}.pmos-solar-section-notes span{font-weight:400;letter-spacing:0;text-transform:none}.pmos-solar-section-notes textarea{min-height:58px;padding:7px 8px;border:1px solid #bfcbd1;border-radius:6px;background:#fff;color:#293944;font:inherit;font-size:11px;resize:vertical}.pmos-customer-contact-divider{height:1px;margin:13px 0;background:#e1e7ea}.pmos-add-location-after-primary{margin-left:7px}@media(max-width:900px){.pmos-solar-equipment-card{grid-template-columns:1fr}.pmos-solar-equipment-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:760px){.pmos-solar-equipment-grid{grid-template-columns:1fr}.pmos-add-location-after-primary{margin-left:0;margin-top:7px}}';
}
...
(function(){
  ...
  function solarLabel(type){return{BOOSTER_PUMP:'Pump',FILTER:'Filter',VALVE_ACTUATOR:'Valve Actuator',CONTROLLER:'Solar Automation',OTHER:'Other Equipment'}[type]||'Solar Equipment'}
  var PMOS_SOLAR_CONTROLLER_CATALOG={Pentair:[{name:'SolarTouch',numbers:['521590','521592','521632']},{name:'SunTouch',numbers:[]}],Hayward:[{name:'AquaSolar GL-235',numbers:['GL-235']}],Goldline:[{name:'AquaSolar GL-235',numbers:['GL-235']}]};
  function solarMakes(type){var source=type==='BOOSTER_PUMP'?(PMOS_EQUIPMENT_CATALOG.pump||{}):type==='FILTER'?(PMOS_FILTER_MODEL_CATALOG||{}):type==='VALVE_ACTUATOR'?(PMOS_VALVE_ACTUATOR_CATALOG||{}):type==='CONTROLLER'?PMOS_SOLAR_CONTROLLER_CATALOG:{};return Object.keys(source)}
  function solarModels(type,make){var key=solarMakes(type).filter(function(value){return value.toLowerCase()===String(make||'').toLowerCase()})[0];if(!key)return[];if(type==='BOOSTER_PUMP'||type==='VALVE_ACTUATOR')return(type==='BOOSTER_PUMP'?PMOS_EQUIPMENT_CATALOG.pump[key]:PMOS_VALVE_ACTUATOR_CATALOG[key])||[];if(type==='CONTROLLER')return PMOS_SOLAR_CONTROLLER_CATALOG[key]||[];if(type==='FILTER'){var groups=PMOS_FILTER_MODEL_CATALOG[key]||{},models=[];Object.keys(groups).forEach(function(filterType){(groups[filterType]||[]).forEach(function(name){models.push({name:filterType+' · '+name,numbers:[]})})});return models}return[]}
  function wireSolarCatalog(row,type){var make=row.querySelector('[data-solar-field="make"]'),model=row.querySelector('[data-solar-field="model"]'),number=row.querySelector('[data-solar-field="modelNumber"]'),modelList=row.querySelector('[data-solar-model-list]'),numberList=row.querySelector('[data-solar-number-list]');if(!make||!model)return;function updateModels(){var models=solarModels(type,make.value);if(modelList)modelList.innerHTML=models.map(function(item){return'<option value="'+esc(item.name)+'">'}).join('');if(models.length&&models.every(function(item){return item.name.toLowerCase()!==String(model.value||'').toLowerCase()}))model.value='';updateNumbers()}function updateNumbers(){var match=solarModels(type,make.value).filter(function(item){return item.name.toLowerCase()===String(model.value||'').toLowerCase()})[0],numbers=match&&match.numbers||[];if(numberList)numberList.innerHTML=numbers.map(function(value){return'<option value="'+esc(typeof value==='string'?value:value.value)+'">'}).join('');if(number)number.value=numbers.length===1?(typeof numbers[0]==='string'?numbers[0]:numbers[0].value):''}make.addEventListener('input',updateModels);model.addEventListener('input',updateNumbers);updateModels()}
  function addSolarEquipment(card,type,data){var list=card&&card.querySelector('[data-solar-list]');if(!list)return;data=data||{};var row=document.createElement('div'),suffix=String(Date.now())+'_'+Math.round(Math.random()*100000),makes=solarMakes(type);row.className='pmos-solar-equipment-card';row.setAttribute('data-solar-equipment',type||'OTHER');row.innerHTML='<div class="pmos-solar-equipment-head"><span>'+solarLabel(type)+'</span><button type="button" class="remove-button" data-solar-remove>Remove</button></div><div class="pmos-solar-equipment-grid"><label>Make<input data-solar-field="make" list="solarMakes_'+suffix+'" placeholder="select or type"><datalist id="solarMakes_'+suffix+'">'+makes.map(function(value){return'<option value="'+esc(value)+'">'}).join('')+'</datalist></label><label>Model<input data-solar-field="model" list="solarModels_'+suffix+'" placeholder="select or type"><datalist id="solarModels_'+suffix+'" data-solar-model-list></datalist></label><label>Model #<input data-solar-field="modelNumber" list="solarNumbers_'+suffix+'" placeholder="select or type"><datalist id="solarNumbers_'+suffix+'" data-solar-number-list></datalist></label><label class="pmos-solar-quantity">Quantity<input data-solar-field="quantity" type="number" min="1"></label></div>';['make','model','modelNumber','quantity'].forEach(function(key){var input=row.querySelector('[data-solar-field="'+key+'"]');if(input)input.value=data[key]||''});if(type!=='VALVE_ACTUATOR')row.querySelector('.pmos-solar-quantity').style.display='none';row.querySelector('[data-solar-remove]').onclick=function(){row.remove()};list.appendChild(row);wireSolarCatalog(row,type);if(typeof enhanceEditableSelects==='function')enhanceEditableSelects(row);['make','model','modelNumber'].forEach(function(key){var input=row.querySelector('[data-solar-field="'+key+'"]');if(input&&data[key])input.value=data[key]})}
  function isSolarHeating(value){return /^solar(?: heating)?$/i.test(String(value||'').trim())}
  window.pmosHydrateSolarEquipmentDetails=function(card,type,details){var mapped={SOLAR_BOOSTER_PUMP:'BOOSTER_PUMP',SOLAR_VALVE_ACTUATOR:'VALVE_ACTUATOR',SOLAR_AUTOMATION:'CONTROLLER'}[type];if(!card||!mapped)return false;var heater=card.querySelector('[data-body-field="heaterType"]');if(heater)heater.value='Solar Heating';configureSolar(card);addSolarEquipment(card,mapped,details||{});return true}
  function ensureSolarPanel(card){var panel=card.querySelector('[data-solar-panel]');if(panel)return panel;var heaterType=card.querySelector('[data-body-field="heaterType"]'),heaterLabel=heaterType&&heaterType.closest('label'),addField=document.createElement('label');addField.className='pmos-solar-add-field';addField.innerHTML='Add Equipment<select class="pmos-solar-add" data-solar-add><option value="" selected>Select equipment</option><option value="BOOSTER_PUMP">Pump</option><option value="VALVE_ACTUATOR">Valve Actuator</option><option value="CONTROLLER">Solar-Specific Controller</option><option value="OTHER">Other Equipment</option></select>';if(heaterLabel&&heaterLabel.parentNode)heaterLabel.insertAdjacentElement('afterend',addField);else card.appendChild(addField);panel=document.createElement('div');panel.className='pmos-solar-panel';panel.setAttribute('data-solar-panel','true');panel.innerHTML='<div class="pmos-solar-head"><div class="pmos-solar-title">Solar Equipment</div><label class="pmos-solar-connected"><input type="checkbox" data-solar-connected> Connected to Equipment Automation</label></div><div class="pmos-solar-list" data-solar-list></div><label class="pmos-solar-section-notes">Solar Equipment Notes <span>optional</span><textarea data-solar-notes placeholder="Notes about the complete solar heating system"></textarea></label>';var grid=card.querySelector('.body-grid'),coverTitle=grid&&Array.prototype.filter.call(grid.querySelectorAll('.unit-title'),function(title){return String(title.textContent||'').trim()==='Covers'})[0];if(grid&&coverTitle)grid.insertBefore(panel,coverTitle);else if(grid)grid.appendChild(panel);else card.appendChild(panel);addField.querySelector('[data-solar-add]').onchange=function(){if(!this.value)return;addSolarEquipment(card,this.value,{});this.value=''};return panel}
  function addSolarTypeOption(input){if(!input)return;var tag=String(input.tagName||'').toLowerCase(),listId=input.getAttribute('list')||input.getAttribute('data-smart-list'),list=tag==='select'?input:listId&&document.getElementById(listId);if(list&&!Array.prototype.some.call(list.options,function(option){return isSolarHeating(option.value)})){var option=document.createElement('option');option.value='Solar Heating';option.textContent='Solar Heating';list.appendChild(option)}}
  function configureSolar(card){if(!card)return;var input=card.querySelector('[data-body-field="heaterType"]');if(!input)return;addSolarTypeOption(input);var solar=isSolarHeating(input.value),panel=ensureSolarPanel(card),addField=card.querySelector('.pmos-solar-add-field');['heaterMake','heaterModel','heaterModelNumber'].forEach(function(key){var field=card.querySelector('[data-body-field="'+key+'"]'),label=field&&field.closest('label');if(solar&&field)field.value='';if(label)label.style.display=solar?'none':''});panel.classList.toggle('open',solar);if(addField)addField.classList.toggle('open',solar)}
  ...
  window.pmosEnsureCustomerBodyEnhancements=ensureBody;window.pmosConfigureSolarHeating=configureSolar;
  function handleSolarHeaterEvent(event){var input=event.target;if(!input||input.getAttribute('data-body-field')!=='heaterType')return;var card=input.closest('.water-body');if(card)configureSolar(card)}
  document.addEventListener('input',handleSolarHeaterEvent);document.addEventListener('change',handleSolarHeaterEvent);
  function collectSolar(card){return Array.prototype.map.call(card.querySelectorAll('[data-solar-equipment]'),function(row){var read=function(key){var input=row.querySelector('[data-solar-field="'+key+'"]');return input?String(input.value||'').trim():''};return{type:row.getAttribute('data-solar-equipment')||'OTHER',make:read('make'),model:read('model'),modelNumber:read('modelNumber'),quantity:read('quantity')}}).filter(function(item){return item.make||item.model||item.modelNumber||item.quantity})}
  function patchBodies(){if(typeof window.addWaterBody==='function'&&!window.addWaterBody.__pmosFormEnhancement){var addBase=window.addWaterBody,addWrapped=function(){var result=addBase.apply(this,arguments),cards=document.querySelectorAll('.water-body'),card=cards[cards.length-1];ensureBody(card);return result};addWrapped.__pmosFormEnhancement=true;window.addWaterBody=addWrapped}if(typeof window.collectWaterBodies==='function'&&!window.collectWaterBodies.__pmosFormEnhancement){var collectBase=window.collectWaterBodies,collectWrapped=function(){var bodies=collectBase.apply(this,arguments)||[],cards=document.querySelectorAll('.water-body');Array.prototype.forEach.call(cards,function(card,index){if(!bodies[index])return;var notes=card.querySelector('[data-body-equipment-notes]');bodies[index].equipmentNotes=notes?String(notes.value||'').trim():'';if(!/^cartridge$/i.test(String(bodies[index].filter&&bodies[index].filter.type||''))&&bodies[index].filter)bodies[index].filter.cartridgeSetNumber='';bodies[index].heater=bodies[index].heater||{};if(isSolarHeating(bodies[index].heater.type)){bodies[index].heater.type='Solar';bodies[index].heater.make='';bodies[index].heater.model='';bodies[index].heater.modelNumber='';bodies[index].heater.solarEquipment=collectSolar(card);var connected=card.querySelector('[data-solar-connected]'),solarNotes=card.querySelector('[data-solar-notes]');bodies[index].heater.connectedToEquipmentAutomation=!!(connected&&connected.checked);bodies[index].heater.solarNotes=solarNotes?String(solarNotes.value||'').trim():''}else{delete bodies[index].heater.solarEquipment;delete bodies[index].heater.connectedToEquipmentAutomation;delete bodies[index].heater.solarNotes}});return bodies};collectWrapped.__pmosFormEnhancement=true;window.collectWaterBodies=collectWrapped}if(typeof window.hydrateBodies==='function'&&!window.hydrateBodies.__pmosFormEnhancement){var hydrateBase=window.hydrateBodies,hydrateWrapped=function(bodies){var result=hydrateBase.apply(this,arguments),rows=bodies||[],cards=document.querySelectorAll('.water-body');Array.prototype.forEach.call(cards,function(card,index){ensureBody(card);var body=rows[index]||{},notes=card.querySelector('[data-body-equipment-notes]'),heaterType=card.querySelector('[data-body-field="heaterType"]');if(notes)notes.value=body.equipmentNotes||'';if(heaterType&&isSolarHeating(body.heater&&body.heater.type))heaterType.value='Solar Heating';configureSolar(card);var connected=card.querySelector('[data-solar-connected]'),solarNotes=card.querySelector('[data-solar-notes]');if(connected)connected.checked=!!(body.heater&&body.heater.connectedToEquipmentAutomation);if(solarNotes)solarNotes.value=body.heater&&body.heater.solarNotes||'';var list=card.querySelector('[data-solar-list]'),solar=body.heater&&body.heater.solarEquipment||[];if(list&&solar.length)list.innerHTML='';(solar||[]).forEach(function(item){addSolarEquipment(card,item.type,item)});var filterType=card.querySelector('[data-body-field="filterType"]');if(filterType&&typeof syncMainFilterCartridgeSet==='function')syncMainFilterCartridgeSet(filterType)});return result};hydrateWrapped.__pmosFormEnhancement=true;window.hydrateBodies=hydrateWrapped}Array.prototype.forEach.call(document.querySelectorAll('.water-body'),ensureBody)}
    ...
    normalizePmosCustomerEditorBodies_ = function(input) {
      ...
      bodies.forEach(function(body, index) {
        ...
        if (solarEquipment.length || /^solar$/i.test(String(body.heater.type || ''))) {
          ...
          body.heater.solarNotes = String(raw.heater && raw.heater.solarNotes || '').trim().slice(0, 5000);
        }
      });
      return bodies;
    };
