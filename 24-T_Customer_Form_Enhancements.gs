...
(function(){
  ...
  function solarLabel(type){return{BOOSTER_PUMP:'Booster Pump',FILTER:'Solar Filter',VALVE_ACTUATOR:'Valve Actuator',CONTROLLER:'Solar Automation',OTHER:'Other Equipment'}[type]||'Solar Equipment'}
  var PMOS_SOLAR_CONTROLLER_CATALOG={Pentair:[{name:'SolarTouch',numbers:['521590','521592','521632']}],Hayward:[{name:'AquaSolar GL-235',numbers:['GL-235']} ]};
  ...
  function ensureSolarPanel(card){var panel=card.querySelector('[data-solar-panel]');if(panel)return panel;panel=document.createElement('div');panel.className='pmos-solar-panel';panel.setAttribute('data-solar-panel','true');panel.innerHTML='<div class="pmos-solar-head"><div class="pmos-solar-title">Solar Heating Equipment</div><label class="pmos-solar-connected"><input type="checkbox" data-solar-connected> Connected to Equipment Automation</label></div><div class="pmos-solar-list" data-solar-list></div><select class="pmos-solar-add" data-solar-add><option value="" selected>+ Add Solar Equipment</option><option value="BOOSTER_PUMP">Booster Pump</option><option value="FILTER">Solar Filter</option><option value="VALVE_ACTUATOR">Valve Actuator</option><option value="CONTROLLER">Solar-Specific Controller</option><option value="OTHER">Other Equipment</option></select>';var grid=card.querySelector('.body-grid'),coverTitle=grid&&Array.prototype.filter.call(grid.querySelectorAll('.unit-title'),function(title){return String(title.textContent||'').trim()==='Covers'})[0];if(grid&&coverTitle)grid.insertBefore(panel,coverTitle);else if(grid)grid.appendChild(panel);else card.appendChild(panel);panel.querySelector('[data-solar-add]').onchange=function(){if(!this.value)return;addSolarEquipment(card,this.value,{});this.value=''};return panel}
