...
body{font-family:Arial,sans-serif;padding:18px;color:#1f2937}h2{margin:0 0 5px}.muted{font-size:13px;color:#6b7280}.stage{margin-top:16px;font-weight:700}.barShell{height:16px;background:#e5e7eb;border-radius:9px;overflow:hidden;margin-top:9px;position:relative}.bar{height:100%;width:35%;background:#2563eb;position:absolute;animation:move 1.25s infinite ease-in-out;border-radius:9px}@keyframes move{0%{left:-35%}100%{left:100%}}.elapsed{text-align:right;font-size:13px;margin-top:5px;color:#4b5563}.result{margin-top:14px;padding:12px;background:#f3f4f6;border-radius:9px;white-space:pre-wrap;max-height:330px;overflow:auto}.buttons{display:flex;gap:8px;margin-top:14px;align-items:center;flex-wrap:wrap}button{border:0;border-radius:8px;padding:9px 13px;font-weight:600;cursor:pointer}.primary{background:#2563eb;color:#fff}.secondary{background:#e5e7eb;color:#111827}.warning{background:#fef3c7;color:#92400e}.complete .bar,.failed .bar{width:100%;left:0;animation:none}button:disabled{opacity:.55;cursor:default}
</style></head><body id="body">
<h2>${escapeHtml_(taskTitle)}</h2><div class="muted">PMOS is checking the current data.</div><div id="stage" class="stage">Working…</div><div class="barShell"><div class="bar"></div></div><div id="elapsed" class="elapsed">Elapsed: 0s</div><div id="result" class="result">Starting ${escapeHtml_(taskTitle)}…</div><div class="buttons"><button id="issuesButton" class="warning" style="display:none">Review errors and warnings</button><button id="deletionsButton" class="warning" style="display:none">Review suggested deletions</button><button id="syncButton" class="primary" style="display:none">Open Calendar Sync</button><button id="closeButton" class="secondary">Close</button></div>
<script>
(function(){
var body=document.getElementById('body'),stage=document.getElementById('stage'),elapsed=document.getElementById('elapsed'),result=document.getElementById('result'),syncButton=document.getElementById('syncButton'),issuesButton=document.getElementById('issuesButton'),deletionsButton=document.getElementById('deletionsButton'),started=Date.now();
...
function done(response){clearInterval(clock);body.className='complete';stage.textContent='Complete';elapsed.textContent='Duration: '+Math.max(1,Math.round((Date.now()-started)/1000))+'s';result.textContent=response&&response.summary?response.summary:'Task completed.';if('${taskType}'==='CALENDAR_AUDIT'&&response){if(response.hasReviewItems)issuesButton.style.display='inline-block';if(Number(response.deletionCandidateCount||0)>0)deletionsButton.style.display='inline-block';if(response.canSync)syncButton.style.display='inline-block';}}
google.script.run.withSuccessHandler(done).withFailureHandler(fail).runPmosTask('${taskType}');
issuesButton.onclick=function(){google.script.run.withFailureHandler(fail).showCalendarAuditIssuesReview();};
deletionsButton.onclick=function(){google.script.run.withFailureHandler(fail).showCalendarDeletionReview();};
...
</script></body></html>`).setWidth(650).setHeight(570);
