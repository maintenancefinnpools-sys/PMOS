 ...
 *
 * Reads verified recurring-series existence and Calendar events within the sync
 * window. It performs no Calendar, spreadsheet, property, trigger, or job writes.
 */

const PMOS_CALENDAR_EVENT_TYPE = Object.freeze({
  RECURRING_ROUTE: 'RECURRING_ROUTE',
  TEMPORARY_VISIT: 'TEMPORARY_VISIT',
  REPAIR_VISIT: 'REPAIR_VISIT',
...
/**
 * Returns the current Calendar reality for the requested range.
 *
 * Options:
 *   startDate: defaults to today at local midnight
 *   endDate: defaults to configured Season End at end of day
 *   includeStartedToday: defaults to false
 */
function readPmosCalendarCurrentState_(settings, registry, options) {
  ...
  return freezePmosCalendarPlannerValue_({
    ...
    range: {
      ...
      includeStartedToday: range.includeStartedToday
    },
...
/** Finds the configured Calendar but never creates it. */
function getExistingConfiguredPmosCalendar_(calendarName) {
  ...
  if (!matches.length) {
    throw new Error(
      'Configured Calendar does not exist: ' + name +
      '. Create it through PMOS setup before running Calendar Preview.'
    );
  }
  if (matches.length > 1) {
    throw new Error(
      'More than one Calendar is named "' + name +
      '". PMOS requires one unambiguous destination Calendar.'
    );
  }
  return matches[0];
}
...
function normalizePmosCalendarReadRange_(settings, options) {
  ...
  const configuredEnd = settings && settings.seasonEnd instanceof Date
    ? new Date(settings.seasonEnd)
    : today;

  const requestedStart = source.startDate instanceof Date
    ? new Date(source.startDate)
    : today;
  const requestedEnd = source.endDate instanceof Date
  ...
  const effectiveStart = !includeStartedToday &&
      requestedStart.getTime() === today.getTime()
  ...
  return {
    ...
    includeStartedToday: includeStartedToday
  };
}
...
function readRegisteredPmosCalendarSeriesState_(calendar, registry) {
  ...
  Object.keys(registry || {}).sort().forEach(function (seriesKey) {
    ...
    records.push({
      state: present
        ? PMOS_CALENDAR_STATE.REGISTERED_PRESENT
        : PMOS_CALENDAR_STATE.REGISTERED_MISSING,
      seriesKey: seriesKey,
      seriesId: String(record.seriesId || ''),
      customerId: String(record.customerId || ''),
      layer: String(record.layer || ''),
      calendarName: String(record.calendarName || ''),
      signature: String(record.signature || ''),
      status: String(record.status || ''),
      actualTitle: present ? safePmosCalendarRead_(series, 'getTitle') : '',
      actualDescription: present ? safePmosCalendarRead_(series, 'getDescription') : '',
...
function readPmosCalendarEventsInRange_(calendar, range) {
  return calendar.getEvents(range.start, range.end).map(function (event) {
    ...
    return {
      ...
      seriesId: readPmosCalendarEventSeriesId_(event),
      ...
      seriesKey: metadata.PMOS_SERIES_KEY || '',
      customerId: metadata.PMOS_CUSTOMER_ID || '',
      temporaryVisitId: metadata.PMOS_TEMP_VISIT_ID || '',
      repairVisitId: metadata.PMOS_REPAIR_VISIT_ID || '',
      managed: metadata.PMOS_MANAGED === 'true' ||
        Boolean(metadata.PMOS_SERIES_KEY || metadata.PMOS_TEMP_VISIT_ID)
    };
  });
}
...
function classifyPmosCalendarEventType_(metadata) {
  ...
  if (declared === PMOS_CALENDAR_EVENT_TYPE.TEMPORARY_VISIT || metadata.PMOS_TEMP_VISIT_ID) {
    return PMOS_CALENDAR_EVENT_TYPE.TEMPORARY_VISIT;
  }
  if (declared === PMOS_CALENDAR_EVENT_TYPE.REPAIR_VISIT || metadata.PMOS_REPAIR_VISIT_ID) {
    return PMOS_CALENDAR_EVENT_TYPE.REPAIR_VISIT;
  }
  if (declared === PMOS_CALENDAR_EVENT_TYPE.RECURRING_ROUTE || metadata.PMOS_SERIES_KEY) {
    return PMOS_CALENDAR_EVENT_TYPE.RECURRING_ROUTE;
  }
