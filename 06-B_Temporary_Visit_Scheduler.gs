...
(function(){
  ...
  function recordAddressMiss(){addressMisses++;addressGoogleFirst=true;}
  function addressIncludesCityText(query){var parts=String(query||'').split(',');return parts.length>1&&String(parts[1]||'').trim().length>=2;}
  function queueAddressSuggestions(){clearTimeout(addressTimer);var query=addressInput.value.trim();if(selectedAddress&&query.toLowerCase()!==String(selectedAddress.address||'').toLowerCase())clearConfirmedAddress();if(query.length<3){closeAddressList();addressMisses=0;addressGoogleFirst=false;showAddressStatus(query?'Keep typing to load address suggestions.':'');return;}if(addressIncludesCityText(query))addressGoogleFirst=true;var token=++addressSequence;showAddressStatus(addressGoogleFirst?'Checking Google for this Ontario address…':'Loading nearby Ontario address suggestions…');addressTimer=setTimeout(function(){google.script.run.withSuccessHandler(function(options){if(token!==addressSequence)return;if(options&&options.length){addressMisses=0;renderAddressOptions(options);return;}recordAddressMiss();closeAddressList();showAddressStatus('Google is checking the Ontario address as you continue typing…');}).withFailureHandler(function(error){if(token!==addressSequence)return;recordAddressMiss();closeAddressList();showAddressStatus(addressGoogleFirst?'Google address lookup is still waiting for a complete Ontario address.':'Address suggestions unavailable: '+(error&&error.message?error.message:String(error)));}).suggestPmosAddresses(query,6,addressGoogleFirst);},160);}
  ...
  function renderDateRecommendations(working){
    ...
    var grid=document.createElement('div');grid.className='dateGrid';allDateOptions.forEach(function(option,index){var button=document.createElement('button');button.type='button';button.className='dateOption';var totalStops=Number(option.customerCount||0)+1,stop='Stop #'+option.position+' out of '+totalStops+(option.position===1?' (first stop)':(option.position===totalStops?' (last stop)':''));var gps=option.roadDataComplete?'<div class="dateMeta"><b>Drive:</b> '+formatMinutes(option.routeDriveMinutes)+' · '+option.routeDistanceKm.toFixed(1)+' km</div><div class="dateMeta"><b>Added:</b> +'+Math.round(option.addedDurationMinutes)+' min · +'+option.addedDistanceKm.toFixed(1)+' km</div>':'<div class="dateMeta"><b>GPS road routing unavailable</b></div>';button.innerHTML='<div class="dateRank">RECOMMENDATION '+(index+1)+'</div><b>'+option.displayDate+'</b><div class="dateMeta"><b>'+(option.displayRating||option.rating)+'</b></div><div class="dateMeta"><b>Week '+option.rotationWeek+' · '+stop+'</b></div>'+gps+'<div class="dateMeta">'+(option.displayReason||option.reason)+'</div>';button.addEventListener('click',function(){selectRecommendedDate(option.date);});grid.appendChild(button);});dateSuggestions.appendChild(grid);
