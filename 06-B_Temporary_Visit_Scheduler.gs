...
(function(){
  ...
  function renderDateRecommendations(working){
    ...
    allDateOptions.forEach(function(option){var button=document.createElement('button');button.type='button';button.className='dateOption';var stop='#'+option.position+(option.position===1?' (first stop)':(option.position===option.customerCount+1?' (last stop)':''));var gps=option.roadDataComplete?'<div class="dateMeta">Drive: '+formatMinutes(option.routeDriveMinutes)+' · '+option.routeDistanceKm.toFixed(1)+' km</div><div class="dateMeta">Added: +'+Math.round(option.addedDurationMinutes)+' min · +'+option.addedDistanceKm.toFixed(1)+' km</div>':'<div class="dateMeta">GPS refinement unavailable</div>';button.innerHTML='<b>'+option.displayDate+' — '+(option.displayRating||option.rating)+'</b><div class="dateMeta">Week '+option.rotationWeek+' • '+option.customerCount+' stops • Placement '+stop+'</div>'+gps+'<div class="dateMeta">'+(option.displayReason||option.reason)+'</div>';button.addEventListener('click',function(){selectRecommendedDate(option.date);});dateSuggestions.appendChild(button);});
