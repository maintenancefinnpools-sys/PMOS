...
<head>
  ...
  <style>
    ...
    button:disabled{opacity:.5;cursor:default}.status{display:none;margin-top:12px;padding:10px;border-radius:8px;background:#f3f4f6;white-space:pre-line}.error{background:#fee2e2;color:#991b1b}.success{background:#dcfce7;color:#166534}@media(max-width:620px){.twoCol,.dateGrid{grid-template-columns:1fr}}
  </style>
</head>
<body>
...
(function(){
  ...
  function selectAddressOption(index){var candidate=addressOptions[index];if(!candidate)return;closeAddressList();showAddressStatus('Confirming address…');google.script.run.withSuccessHandler(function(result){selectedAddress=result;addressInput.value=result.address;addressDetails.textContent='Confirmed: '+result.address;addressDetails.style.display='block';showAddressStatus('Address confirmed. Calculating GPS route placements…');searchOffset=0;allDateOptions=[];refreshDateRecommendations(false);queuePlacementSuggestions();}).withFailureHandler(function(error){clearConfirmedAddress();showAddressStatus(error&&error.message?error.message:String(error));}).confirmPmosSelectedAddress(candidate);}
  function queueAddressSuggestions(){clearTimeout(addressTimer);var query=addressInput.value.trim();if(selectedAddress&&query.toLowerCase()!==String(selectedAddress.address||'').toLowerCase())clearConfirmedAddress();if(query.length<3){closeAddressList();showAddressStatus(query?'Keep typing to load address suggestions.':'');return;}var token=++addressSequence;showAddressStatus('Loading address suggestions…');addressTimer=setTimeout(function(){google.script.run.withSuccessHandler(function(options){if(token!==addressSequence)return;renderAddressOptions(options);}).withFailureHandler(function(error){if(token!==addressSequence)return;closeAddressList();showAddressStatus('Address suggestions unavailable: '+(error&&error.message?error.message:String(error)));}).suggestPmosAddresses(query,6);},160);}
  ...
  function renderDateRecommendations(working){
    ...
    var grid=document.createElement('div');grid.className='dateGrid';allDateOptions.forEach(function(option,index){var button=document.createElement('button');button.type='button';button.className='dateOption';var stop='#'+option.position+(option.position===1?' (first stop)':(option.position===option.customerCount+1?' (last stop)':''));var gps=option.roadDataComplete?'<div class="dateMeta"><b>Drive:</b> '+formatMinutes(option.routeDriveMinutes)+' · '+option.routeDistanceKm.toFixed(1)+' km</div><div class="dateMeta"><b>Added:</b> +'+Math.round(option.addedDurationMinutes)+' min · +'+option.addedDistanceKm.toFixed(1)+' km</div>':'<div class="dateMeta">GPS refinement unavailable</div>';button.innerHTML='<div class="dateRank">RECOMMENDATION '+(index+1)+'</div><b>'+option.displayDate+'</b><div class="dateMeta"><b>'+(option.displayRating||option.rating)+'</b></div><div class="dateMeta">Week '+option.rotationWeek+' · '+option.customerCount+' stops</div><div class="dateMeta"><b>Placement:</b> '+stop+'</div>'+gps+'<div class="dateMeta">'+(option.displayReason||option.reason)+'</div>';button.addEventListener('click',function(){selectRecommendedDate(option.date);});grid.appendChild(button);});dateSuggestions.appendChild(grid);
