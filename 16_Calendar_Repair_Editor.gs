...
function card(item){const c=document.createElement('div');c.className='stop';c.draggable=true;c.dataset.id=item.id;c.dataset.customerId=item.customerId||'';c.textContent=item.title;const x=document.createElement('button');x.className='remove';x.type='button';x.textContent='×';x.title='Remove from this repair preview';x.onclick=e=>{e.stopPropagation();c.remove()};c.appendChild(x);c.ondragstart=()=>dragged=c;c.ondragend=()=>dragged=null;return c}
function render(){board.innerHTML='';lanes.forEach(l=>{const lane=document.createElement('div');lane.className='lane';lane.dataset.date=l.date;lane.innerHTML='<h3>'+l.day+'<br>'+l.date+'</h3>';lane.ondragover=e=>e.preventDefault();lane.ondrop=e=>{e.preventDefault();if(dragged)lane.appendChild(dragged)};items.filter(i=>i.date===l.date).sort((a,b)=>a.order-b.order).forEach(i=>lane.appendChild(card(i)));board.appendChild(lane)})}
function addCustomer(){const customer=customers[Number(customerSelect.value)];const lane=document.querySelector('.lane[data-date="'+dateSelect.value+'"]');if(!customer||!lane)return;const item={id:'added-'+Date.now()+'-'+Math.random().toString(16).slice(2),customerId:customer.customerId||'',title:customer.title};lane.appendChild(card(item));status.textContent=customer.title+' added to the preview.'}
